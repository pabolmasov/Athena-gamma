    ...
    for (int k=pmb->ks; k<=pmb->ke; ++k) {
        for (int j=pmb->js; j<=pmb->je; ++j) {
            ...
            for (int i=pmb->is; i<=pmb->ie; ++i) {
                ...
                if (addpot){
                    phic += x;
                    phil += 0.5 * (pmb->pcoord->x1v(i-1)+x);
                    phir += 0.5 * (pmb->pcoord->x1v(i+1)+x);
                }
                    cons(IM1,k,j,i) -= dtodx1*prim(IDN,k,j,i)*(phir-phil);
                
                
                if (NON_BAROTROPIC_EOS){
                    Real x1 = x - pmb->pcoord->dx1v(i)/2.;
                    x1 = x + pmb->pcoord->dx1v(i)/2.;
                    Real flux1l = flux[X1DIR](IDN,k,j,i);
 
                    Real flux1r = flux[X1DIR](IDN,k,j,i+1);

                    cons(IEN,k,j,i) -= dtodx1*(flux1l*(phic - phil) +
                                               flux1r*(phir - phic));
                }
